...
const PropertyCard = ({ item, variant = 'compact', axis = 'vertical' }: PropertyCardProps) => {
  ...
  //   const image = item.image_url || 'image.png';
  const image = 'image.png';

  return (
    <figure onClick={() => handleDetail(item.id)} className={`rounded-xl cursor-pointer transition duration-300 ease-in overflow-hidden ${axis === 'horizontal' ? 'grid grid-cols-2' : 'grid grid-cols-1'}`}>
      ...
      <div className="p-3">
        ...
        <div className="flex justify-between items-center gap-2 mt-2">
          <p className="text-md font-bold text-blue-600">{item.price_display}</p>
          <button className="text-xs border border-slate-300 rounded-full py-1 px-3 hover:border-slate-800 hover:bg-slate-800 hover:text-white">book now</button>
        </div>
      </div>
    </figure>
  );
