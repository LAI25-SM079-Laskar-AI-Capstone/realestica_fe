...
import { createFacilityMapping, createPointOfInterestMapping } from '../utils/handler';

const HousePredict: React.FC = () => {
  const { predictPrice, prediction, isPending, isError, error, isSuccess } = usePredict();

  const handleSubmitForm = (formData: BasicFormData, selectedFacilities: string[], selectedPointsOfInterest: string[]) => {
  ...
  return (
    <div className="container mx-auto p-4">
      <HousePredictForm onSubmit={handleSubmitForm} isSubmitting={isPending} predictionResult={isSuccess ? prediction : undefined} error={isError ? error : undefined} />
    </div>
  );
};
